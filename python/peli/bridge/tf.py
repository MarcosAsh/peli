...
def to_tensorflow(peli_arr):
    """from peli to tensorflow, no copy"""
    tfdl = try_import_tfdl()
    from tensorflow.python import pywrap_tfe
    from tensorflow.python.eager import context
    ctx = context.context()
    ctx.ensure_initialized()
    return pywrap_tfe.TFE_FromDlpackCapsule(peli_arr.to_dlpack(), ctx._handle)

def from_tensorflow(tf_tensor):
    """from tensorflow to peli, no copy"""
    tfdl = try_import_tfdl()
