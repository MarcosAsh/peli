...
def find_lib_path(name=None, search_path=None, optional=False):
    ...
    # NB: This will either be the source directory (if PELI is run
    # inplace) or the install directory (if PELI is installed).
    # An installed PELI's curr_path will look something like:
    #   $PREFIX/lib/python3.6/site-packages/peli/_ffi
    ...
    if os.environ.get('PELI_LIBRARY_PATH', None):
        dll_path.append(os.environ['PELI_LIBRARY_PATH'])
    ...
    if name is not None:
        ...
        if sys.platform.startswith('win32'):
            lib_dll_path = [os.path.join(p, 'libpeli.dll') for p in dll_path] +\
                           [os.path.join(p, 'peli.dll') for p in dll_path]
        elif sys.platform.startswith('darwin'):
            lib_dll_path = [os.path.join(p, 'libpeli.dylib') for p in dll_path]
        else:
            lib_dll_path = [os.path.join(p, 'libpeli.so') for p in dll_path]
...
# The following line is set by peli/python/update_version.py
__version__ = "0.1.0"
