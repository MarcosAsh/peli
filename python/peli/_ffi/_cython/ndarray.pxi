...
cdef void _c_dlpack_deleter(object pycaps):
    ...
    if pycapsule.PyCapsule_IsValid(pycaps, _c_str_dltensor):
        ...
        PELIDLManagedTensorCallDeleter(dltensor)
...
def _from_dlpack(object dltensor):
    ...
    if pycapsule.PyCapsule_IsValid(dltensor, _c_str_dltensor):
        ...
        CALL(PELIArrayFromDLPack(ptr, &chandle))
...
cdef class NDArrayBase:
    ...
    property handle:
        def __get__(self):
            if self.chandle == NULL:
                ...
                return ctypes.cast(
                    <unsigned long long>self.chandle, PELIArrayHandle)
    ...
    def __dealloc__(self):
        if self.c_is_view == 0:
            CALL(PELIArrayFree(self.chandle))
    ...
    def to_dlpack(self):
        ...
        CALL(PELIArrayToDLPack(self.chandle, &dltensor))
...
def _reg_extension(cls, fcreate):
    ...
    _PELI_COMPATS += (cls,)
    if fcreate:
        _PELI_EXT_RET[cls._peli_tcode] = fcreate
