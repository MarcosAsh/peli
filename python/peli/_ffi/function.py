...
try:
    ...
    from ._ctypes.function import convert_to_peli_func
...
class Function(_FunctionBase):
    ...
    Function plays an key role to bridge front and backend in PELI.
    ...
    PELI backend also registers and exposes its API as Functions.
    For example, the developer function exposed in peli.ir_pass are actually
    C++ functions that are registered as PackedFunc

    The following are list of common usage scenario of peli.Function.
    ...
    See Also
    --------
    peli.register_func: How to register global function.
    peli.get_global_func: How to get global function.
    """
...
class ModuleBase(object):
    ...
    def __init__(self, handle):
        self.handle = handle
        self._entry = None
        self.entry_name = "__peli_main__"

    def __del__(self):
        check_call(_LIB.PELIModFree(self.handle))
    ...
    def get_function(self, name, query_imports=False):
        ...
        check_call(_LIB.PELIModGetFunction(
    ...
    def import_module(self, module):
        ...
        module : Module
            The other module.
        """
        check_call(_LIB.PELIModImport(self.handle, module.handle))
...
def register_func(func_name, f=None, override=False):
    ...
    from C++ backend, or in the compiled PELI code.
    ...
    .. code-block:: python
      ...
      f = peli.get_global_func("my_packed_func")
      assert isinstance(f, peli.nd.Function)
      y = f(*targs)
      assert y == 10
    """
    ...
    def register(myf):
        ...
        if not isinstance(myf, Function):
            myf = convert_to_peli_func(myf)
        check_call(_LIB.PELIFuncRegisterGlobal(
...
def get_global_func(name, allow_missing=False):
    ...
    check_call(_LIB.PELIFuncGetGlobal(c_str(name), ctypes.byref(handle)))
    if handle.value:
...
def list_global_func_names():
    ...
    check_call(_LIB.PELIFuncListGlobalNames(ctypes.byref(size),
                                           ctypes.byref(plist)))
    fnames = []
...
def extract_ext_funcs(finit):
    ...
    finit : ctypes function
        a ctypes that takes signature of PELIExtensionDeclarer
    ...
    def _list(name, func):
        fdict[name] = func
    myf = convert_to_peli_func(_list)
...
def _init_api(namespace, target_module_name=None):
    ...
    if namespace.startswith("peli."):
        _init_api_prefix(target_module_name, namespace[len("peli."):])
    else:
...
def _init_api_prefix(module_name, prefix):
    ...
    for name in list_global_func_names():
        ...
        ff = _get_api(f)
        ff.__name__ = fname
        ff.__doc__ = ("PELI PackedFunc %s. " % fname)
...
def _init_internal_api():
    for name in list_global_func_names():
        if not name.startswith("_"):
            continue
        target_module = sys.modules["peli._api_internal"]
        fname = name
        if fname.find(".") != -1:
            print('Warning: invalid API name "%s".' % fname)
            continue
