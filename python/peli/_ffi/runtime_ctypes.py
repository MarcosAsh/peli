...
peli_shape_index_t = ctypes.c_int64
...
class PELIByteArray(ctypes.Structure):
    """Temp data structure for byte array."""
    _fields_ = [("data", ctypes.POINTER(ctypes.c_byte)),
                ("size", ctypes.c_size_t)]

class PELIType(ctypes.Structure):
    """PELI datatype structure"""
    ...
    def __new__(cls, type_str):
        ...
        inst = super(PELIType, cls).__new__(PELIType)
    ...
    def __repr__(self):
        x = "%s%d" % (PELIType.CODE2STR[self.type_code], self.bits)
        if self.lanes != 1:
            x += "x%d" % self.lanes
        return x
...
class PELIContext(ctypes.Structure):
    """PELI context strucure."""
    ...
    def __new__(cls, device_type, device_id):
        ...
        inst = super(PELIContext, cls).__new__(PELIContext)
    ...
    def sync(self):
        ...
        check_call(_LIB.PELISynchronize(self.device_type, self.device_id, None))

    def __eq__(self, other):
        return (isinstance(other, PELIContext) and
    ...
    def __repr__(self):
        if self.device_type >= RPC_SESS_MASK:
            ...
            return "remote[%d]:%s(%d)" % (
                tbl_id, PELIContext.MASK2STR[dev_type], self.device_id)
        return "%s(%d)" % (
            PELIContext.MASK2STR[self.device_type], self.device_id)

    def __hash__(self):
        return hash((self.device_type, self.device_id))


class PELIArray(ctypes.Structure):
    """PELIValue in C API"""
    _fields_ = [("data", ctypes.c_void_p),
                ("ctx", PELIContext),
                ("ndim", ctypes.c_int),
                ("dtype", PELIType),
                ("shape", ctypes.POINTER(peli_shape_index_t)),
                ("strides", ctypes.POINTER(peli_shape_index_t)),
                ("byte_offset", ctypes.c_uint64)]

PELIArrayHandle = ctypes.POINTER(PELIArray)
