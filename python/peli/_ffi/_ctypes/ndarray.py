...
from ..runtime_ctypes import PELIArrayHandle
from .types import RETURN_SWITCH, C_TO_PY_ARG_SWITCH, _wrap_arg_func, _return_handle


PELIPyCapsuleDestructor = ctypes.CFUNCTYPE(None, ctypes.c_void_p)
...
def _from_dlpack(dltensor):
    ...
    if ctypes.pythonapi.PyCapsule_IsValid(dltensor, _c_str_dltensor):
        ...
        check_call(_LIB.PELIArrayFromDLPack(ptr, ctypes.byref(handle)))
        ctypes.pythonapi.PyCapsule_SetName(dltensor, _c_str_used_dltensor)
        ctypes.pythonapi.PyCapsule_SetDestructor(dltensor, PELIPyCapsuleDestructor(0))
...
def _dlpack_deleter(pycapsule):
    ...
    if ctypes.pythonapi.PyCapsule_IsValid(pycapsule, _c_str_dltensor):
        ...
        #   work out always.
        ptr = ctypes.cast(ptr, ctypes.c_void_p)
        _LIB.PELIDLManagedTensorCallDeleter(ptr)
        ctypes.pythonapi.PyCapsule_SetDestructor(pycapsule, PELIPyCapsuleDestructor(0))

_c_dlpack_deleter = PELIPyCapsuleDestructor(_dlpack_deleter)
...
class NDArrayBase(object):
    ...
    def __init__(self, handle, is_view=False):
        ...
        ----------
        handle : PELIArrayHandle
            the handle to the underlying C++ PELIArray
        """
        self.handle = handle
    ...
    def __del__(self):
        if not self.is_view and _LIB:
            check_call(_LIB.PELIArrayFree(self.handle))
    ...
    def to_dlpack(self):
        ...
        """
        ptr = ctypes.c_void_p()
        check_call(_LIB.PELIArrayToDLPack(self.handle, ctypes.byref(ptr)))
        return ctypes.pythonapi.PyCapsule_New(ptr, _c_str_dltensor, _c_dlpack_deleter)


def _make_array(handle, is_view):
    handle = ctypes.cast(handle, PELIArrayHandle)
    return _CLASS_NDARRAY(handle, is_view)

_PELI_COMPATS = ()

def _reg_extension(cls, fcreate):
    global _PELI_COMPATS
    _PELI_COMPATS += (cls,)
    if fcreate:
        fret = lambda x: fcreate(_return_handle(x))
        RETURN_SWITCH[cls._peli_tcode] = fret
        C_TO_PY_ARG_SWITCH[cls._peli_tcode] = _wrap_arg_func(fret, cls._peli_tcode)
