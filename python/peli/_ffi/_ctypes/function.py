...
from ..runtime_ctypes import PELIType, PELIByteArray, PELIContext
from . import ndarray as _nd
from .ndarray import NDArrayBase, _make_array
from .types import PELIValue, TypeCode
from .types import PELIPackedCFunc, PELICFuncFinalizer
from .types import RETURN_SWITCH, C_TO_PY_ARG_SWITCH, _wrap_arg_func

FunctionHandle = ctypes.c_void_p
ModuleHandle = ctypes.c_void_p
PELIRetValueHandle = ctypes.c_void_p
...
PELI_FREE_PYOBJ = PELICFuncFinalizer(_ctypes_free_resource)
ctypes.pythonapi.Py_IncRef(ctypes.py_object(PELI_FREE_PYOBJ))

def convert_to_peli_func(pyfunc):
    ...
    def cfun(args, type_codes, num_args, ret, _):
        ...
        try:
            ...
            _LIB.PELIAPISetLastError(c_str(msg))
            return -1
        ...
        if rv is not None:
            ...
            temp_args = []
            values, tcodes, _ = _make_peli_args((rv,), temp_args)
            if not isinstance(ret, PELIRetValueHandle):
                ret = PELIRetValueHandle(ret)
            check_call(_LIB.PELICFuncSetReturn(ret, values, tcodes, ctypes.c_int(1)))
            _ = temp_args
            _ = rv
        return 0

    handle = FunctionHandle()
    f = PELIPackedCFunc(cfun)
    # NOTE: We will need to use python-api to increase ref count of the f
    # PELI_FREE_PYOBJ will be called after it is no longer needed.
    pyobj = ctypes.py_object(f)
    ctypes.pythonapi.Py_IncRef(pyobj)
    check_call(_LIB.PELIFuncCreateFromCFunc(
        f, pyobj, PELI_FREE_PYOBJ, ctypes.byref(handle)))
    return _CLASS_FUNCTION(handle, False)


def _make_peli_args(args, temp_args):
    """Pack arguments into c args peli call accept"""
    num_args = len(args)
    values = (PELIValue * num_args)()
    ...
    for i, arg in enumerate(args):
        if arg is None:
        ...
        elif isinstance(arg, _nd._PELI_COMPATS):
            values[i].v_handle = ctypes.c_void_p(arg._peli_handle)
            type_codes[i] = arg.__class__._peli_tcode
            ...
            values[i].v_ctx = arg
            type_codes[i] = TypeCode.PELI_CONTEXT
            ...
            type_codes[i] = TypeCode.STR
        # NOTE(minjie): module is not used in PELI
...
class FunctionBase(object):
    ...
    def __del__(self):
        if not self.is_global and _LIB is not None:
            check_call(_LIB.PELIFuncFree(self.handle))
    ...
    def __call__(self, *args):
        ...
        """
        temp_args = []
        values, tcodes, num_args = _make_peli_args(args, temp_args)
        ret_val = PELIValue()
        ret_tcode = ctypes.c_int()
        check_call(_LIB.PELIFuncCall(
