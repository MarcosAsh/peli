...
from ..runtime_ctypes import PELIByteArray, TypeCode

class PELIValue(ctypes.Union):
    """PELIValue in C API"""
...
PELIPackedCFunc = ctypes.CFUNCTYPE(
    ctypes.c_int,
    ctypes.POINTER(PELIValue),
...
PELICFuncFinalizer = ctypes.CFUNCTYPE(
    None,
    ctypes.c_void_p)
...
def _wrap_arg_func(return_f, type_code):
    ...
    def _wrap_func(x):
        check_call(_LIB.PELICbArgToReturn(ctypes.byref(x), tcode))
        return return_f(x)
    return _wrap_func
