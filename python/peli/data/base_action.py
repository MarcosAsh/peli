...
class VideoClsCustom(object):
    ...
    ctx : peli.Context, default is cpu(0)
    ...
    def __getitem__(self, index):
        ...
        peli_vr = VideoReader(video_name, width=self.new_width, height=self.new_height, ctx=self.ctx if ctx is None else ctx)
        duration = len(peli_vr)
        ...
        if self.slowfast:
            clip_input = self._video_TSN_peli_slowfast_loader(directory, peli_vr, duration, segment_indices, skip_offsets)
        else:
            clip_input = self._video_TSN_peli_batch_loader(directory, peli_vr, duration, segment_indices, skip_offsets)
    ...
    def _video_TSN_peli_loader(self, directory, video_reader, duration, indices, skip_offsets):
        sampled_list = []
    ...
    def _video_TSN_peli_batch_loader(self, directory, video_reader, duration, indices, skip_offsets):
        sampled_list = []
        frame_id_list = []
        for seg_ind in indices:
            offset = int(seg_ind)
    ...
    def _video_TSN_peli_slowfast_loader(self, directory, video_reader, duration, indices, skip_offsets):
        sampled_list = []
        frame_id_list = []
        for seg_ind in indices:
            fast_id_list = []
