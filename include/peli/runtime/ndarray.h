 ...
 * \file peli/runtime/ndarray.h
 * \brief Abstract device memory management API
 */
#ifndef PELI_RUNTIME_NDARRAY_H_
#define PELI_RUNTIME_NDARRAY_H_
...
class NDArray {
 public:
   ...
   */
  PELI_DLL NDArray CreateOffsetView(
   ...
   */
  PELI_DLL DLManagedTensor* ToDLPack() const;
  /*!
   ...
   */
  PELI_DLL static NDArray Empty(std::vector<int64_t> shape,
                               DLDataType dtype,
                               DLContext ctx);
  /*!
   ...
   */
  PELI_DLL static NDArray FromDLPack(DLManagedTensor* tensor);
  /*!
   ...
   */
  PELI_DLL static void CopyFromTo(
      DLTensor* from, DLTensor* to, PELIStreamHandle stream = nullptr);
 ...
 private:
  ...
  friend class peli::NDArrayPool;
  friend class peli::VideoReader;
  friend class peli::VideoLoader;
  friend class peli::AudioReader;
};
...
inline size_t GetDataSize(const DLTensor& arr) {
  size_t size = 1;
  for (peli_index_t i = 0; i < arr.ndim; ++i) {
...
constexpr uint64_t kPELINDArrayMagic = 0xDD5E40F096B4A13F;

inline bool SaveDLTensor(dmlc::Stream* strm,
                         DLTensor* tensor) {
  uint64_t header = kPELINDArrayMagic, reserved = 0;
  strm->Write(header);
  strm->Write(reserved);
...
inline bool NDArray::Load(dmlc::Stream* strm) {
  ...
  CHECK(strm->Read(&reserved))
      << "Invalid DLTensor file format";
  CHECK(header == kPELINDArrayMagic)
      << "Invalid DLTensor file format";
  DLContext ctx;
  int ndim;
  DLDataType dtype;
