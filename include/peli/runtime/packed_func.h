 ...
 * \file peli/runtime/packed_func.h
 * \brief Type-erased function used across PELI API.
 */
#ifndef PELI_RUNTIME_PACKED_FUNC_H_
#define PELI_RUNTIME_PACKED_FUNC_H_
...
// Whether use PELI runtime in header only mode.
#ifndef PELI_RUNTIME_HEADER_ONLY
#define PELI_RUNTIME_HEADER_ONLY 0
#endif
 ...
 *  It is the unified function function type of PELI.
 *  It corresponds to PELIFunctionHandle in C runtime API.
 */
class PackedFunc {
 public:
  /*!
   ...
   *   void MyPackedFunc(PELIArgs args, PELIRetValue* rv) {
   ...
   * \endcode
   */
  using FType = std::function<void (PELIArgs args, PELIRetValue* rv)>;
  ...
  inline PELIRetValue operator()(Args&& ...args) const;
  /*!
   * \brief Call the function in packed format.
   * \param args The arguments
   * \param rv The return value.
   */
  inline void CallPacked(PELIArgs args, PELIRetValue* rv) const;
 ...
 * - It can be assigned to PELIRetValue.
...
class TypedPackedFunc<R(Args...)> {
 public:
   ...
   * Example usage:
   * \code
   * PackedFunc packed([](PELIArgs args, PELIRetValue *rv) {
   *   int x = args[0];
   *   *rv = x + 1;
   *  });
  ...
  /*!
   * \brief constructor from PELIRetValue
   * \param value The PELIRetValue
   */
  inline TypedPackedFunc(const PELIRetValue& value);  // NOLINT(*)
  /*!
   * \brief constructor from PELIArgValue
   * \param value The PELIArgValue
   */
  inline TypedPackedFunc(const PELIArgValue& value);  // NOLINT(*)
  /*!
...
/*! \brief Arguments into PELI functions. */
class PELIArgs {
 public:
  const PELIValue* values;
   ...
   */
  PELIArgs(const PELIValue* values,
          const int* type_codes,
          int num_args)
      : values(values),
   ...
   */
  inline PELIArgValue operator[](int i) const;
};
...
/*!
 * \brief convert a string to PELI type.
 * \param s The string to be converted.
 * \return The corresponding peli type.
 */
inline PELIType String2PELIType(std::string s);

/*!
 * \brief convert a PELI type to string.
 * \param t The type to be converted.
 * \return The corresponding peli type in string.
 */
inline std::string PELIType2String(PELIType t);

// macro to check type code.
#define PELI_CHECK_TYPE_CODE(CODE, T)                           \
  CHECK_EQ(CODE, T) << " expected "                            \
  << TypeCode2Str(T) << " but get " << TypeCode2Str(CODE)      \

/*!
 * \brief Type traits to mark if a class is peli extension type.
 *
 * To enable extension type in C++ must be register () ed via marco.
 * PELI_REGISTER_EXT_TYPE(TypeName) after defining this with this traits.
 *
 * Extension class can be passed and returned via PackedFunc in all peli runtime.
...
class ExtTypeVTable {
 public:
   ...
   * \return The registered vtable.
   */
  PELI_DLL static ExtTypeVTable* Get(int type_code);

 private:
  // Internal registration function.
  PELI_DLL static ExtTypeVTable* RegisterInternal(int type_code, const ExtTypeVTable& vt);
};
...
class PELIPODValue_ {
 public:
  operator double() const {
    ...
    PELI_CHECK_TYPE_CODE(type_code_, kDLFloat);
    return value_.v_float64;
  }
  operator int64_t() const {
    PELI_CHECK_TYPE_CODE(type_code_, kDLInt);
  ...
  operator void*() const {
    ...
    PELI_CHECK_TYPE_CODE(type_code_, kHandle);
  ...
  operator NDArray() const {
    ...
    PELI_CHECK_TYPE_CODE(type_code_, kNDArrayContainer);
    return NDArray(static_cast<NDArray::Container*>(value_.v_handle));
  }
  operator PELIContext() const {
    PELI_CHECK_TYPE_CODE(type_code_, kPELIContext);
    return value_.v_ctx;
  }
 ...
 protected:
  friend class PELIArgsSetter;
  friend class PELIRetValue;
  PELIPODValue_() : type_code_(kNull) {}
  PELIPODValue_(PELIValue value, int type_code)
 ...
 *  Containing both type_code and PELIValue
 *
 *  Provides utilities to do type cast into other types.
 */
class PELIArgValue : public PELIPODValue_ {
 public:
  /*! \brief default constructor */
  PELIArgValue() {}
  /*!
   ...
   * \param type_code The type code.
   */
  PELIArgValue(PELIValue value, int type_code)
      : PELIPODValue_(value, type_code) {
  }
  // reuse converter from parent
  using PELIPODValue_::operator double;
  using PELIPODValue_::operator int64_t;
  using PELIPODValue_::operator uint64_t;
  using PELIPODValue_::operator int;
  using PELIPODValue_::operator bool;
  using PELIPODValue_::operator void*;
  using PELIPODValue_::operator DLTensor*;
  using PELIPODValue_::operator NDArray;
  using PELIPODValue_::operator PELIContext;

  // conversion operator.
  operator std::string() const {
    if (type_code_ == kPELIType) {
      return PELIType2String(operator PELIType());
    } else if (type_code_ == kBytes) {
      PELIByteArray* arr = static_cast<PELIByteArray*>(value_.v_handle);
      return std::string(arr->data, arr->size);
    } else {
      PELI_CHECK_TYPE_CODE(type_code_, kStr);
      return std::string(value_.v_str);
    }
  }
  operator PELIType() const {
    if (type_code_ == kStr) {
      return String2PELIType(operator std::string());
    }
    // None type
    if (type_code_ == kNull) {
      PELIType t;
    ...
    PELI_CHECK_TYPE_CODE(type_code_, kPELIType);
    return value_.v_type;
  }
  operator PackedFunc() const {
    if (type_code_ == kNull) return PackedFunc();
    PELI_CHECK_TYPE_CODE(type_code_, kFuncHandle);
  ...
  operator Module() const {
    PELI_CHECK_TYPE_CODE(type_code_, kModuleHandle);
    return *ptr<Module>();
  }
  const PELIValue& value() const {
    return value_;
  }
  ...
  inline operator peli::Integer() const;
 ...
 *  Unlike PELIArgValue, which only holds reference and do not delete
 *  the underlying container during destruction.
 *
 *  PELIRetValue holds value and will manage the underlying containers
 *  when it stores a complicated data type.
 */
class PELIRetValue : public PELIPODValue_ {
 public:
  /*! \brief default constructor */
  PELIRetValue() {}
  /*!
   ...
   */
  PELIRetValue(PELIRetValue&& other)
      : PELIPODValue_(other.value_, other.type_code_) {
  ...
  using PELIPODValue_::operator PELIContext;
  using PELIPODValue_::operator NDArray;
  PELIRetValue(const PELIRetValue& other) : PELIPODValue_() {
    this->Assign(other);
  }
  ...
  // Assign operators
  PELIRetValue& operator=(PELIRetValue&& other) {
    this->Clear();
    value_ = other.value_;
  ...
  PELIRetValue& operator=(double value) {
    this->SwitchToPOD(kDLFloat);
    value_.v_float64 = value;
    return *this;
  }
  PELIRetValue& operator=(std::nullptr_t value) {
    this->SwitchToPOD(kNull);
    value_.v_handle = value;
    return *this;
  }
  PELIRetValue& operator=(void* value) {
    this->SwitchToPOD(kHandle);
    value_.v_handle = value;
    return *this;
  }
  PELIRetValue& operator=(int64_t value) {
    this->SwitchToPOD(kDLInt);
    value_.v_int64 = value;
    return *this;
  }
  PELIRetValue& operator=(int value) {
    this->SwitchToPOD(kDLInt);
    value_.v_int64 = value;
    return *this;
  }
  PELIRetValue& operator=(PELIContext value) {
    this->SwitchToPOD(kPELIContext);
    value_.v_ctx = value;
    return *this;
  }
  PELIRetValue& operator=(PELIType t) {
    this->SwitchToPOD(kPELIType);
    value_.v_type = t;
    return *this;
  }
  PELIRetValue& operator=(bool value) {
    this->SwitchToPOD(kDLInt);
    value_.v_int64 = value;
    return *this;
  }
  PELIRetValue& operator=(std::string value) {
    this->SwitchToClass(kStr, value);
    return *this;
  }
  PELIRetValue& operator=(PELIByteArray value) {
    this->SwitchToClass(kBytes, std::string(value.data, value.size));
    return *this;
  }
  PELIRetValue& operator=(NDArray other) {
    this->Clear();
    type_code_ = kNDArrayContainer;
  ...
  PELIRetValue& operator=(PackedFunc f) {
    this->SwitchToClass(kFuncHandle, f);
    return *this;
  }
  template<typename FType>
  PELIRetValue& operator=(const TypedPackedFunc<FType>& f) {
    return operator=(f.packed());
  }
  PELIRetValue& operator=(Module m) {
    this->SwitchToClass(kModuleHandle, m);
    return *this;
  }
  PELIRetValue& operator=(const PELIRetValue& other) {  // NOLINT(*0
    this->Assign(other);
    return *this;
  }
  PELIRetValue& operator=(const PELIArgValue& other) {
    this->Assign(other);
    return *this;
  }
  template<typename T,
  ...
  PELIRetValue& operator=(const T& other) {
   ...
   */
  void MoveToCHost(PELIValue* ret_value,
                   int* ret_type_code) {
  ...
  const PELIValue& value() const {
    CHECK(type_code_ != kNodeHandle &&
          ...
          type_code_ != kStr) << "PELIRetValue.value can only be used for POD data";
    return value_;
  }
  // NodeRef related extenstions: in peli/packed_func_ext.h
  template<typename T,
           typename = typename std::enable_if<
             std::is_class<T>::value>::type>
  inline operator T() const;
  template<typename TNodeRef>
  inline TNodeRef AsNodeRef() const;
  inline PELIRetValue& operator=(const NodeRef& other);
  inline PELIRetValue& operator=(const NodePtr<Node>& other);
  // type related
  inline operator HalideIR::Type() const;
  inline PELIRetValue& operator=(const HalideIR::Type& other);
...
inline const char* TypeCode2Str(int type_code) {
  switch (type_code) {
    ...
    case kPELIType: return "PELIType";
    case kPELIContext: return "PELIContext";
...
inline std::ostream& operator<<(std::ostream& os, PELIType t) {  // NOLINT(*)
...
inline PELIArgValue PELIArgs::operator[](int i) const {
  CHECK_LT(i, num_args)
      << "not enough argument passed, "
      << num_args << " passed"
      << " but request arg[" << i << "].";
  return PELIArgValue(values[i], type_codes[i]);
}

inline int PELIArgs::size() const {
  return num_args;
}

inline void PackedFunc::CallPacked(PELIArgs args, PELIRetValue* rv) const {
  body_(args, rv);
}
...
class PELIArgsSetter {
 public:
  PELIArgsSetter(PELIValue* values, int* type_codes)
  ...
  void operator()(size_t i, const PELIArgValue& value) const {
  ...
  void operator()(size_t i, PELIContext value) const {
    values_[i].v_ctx = value;
    type_codes_[i] = kPELIContext;
  }
  void operator()(size_t i, PELIType value) const {
    values_[i].v_type = value;
    type_codes_[i] = kPELIType;
  }
  ...
  void operator()(size_t i, const PELIByteArray& value) const {  // NOLINT(*)
    values_[i].v_handle = const_cast<PELIByteArray*>(&value);
  ...
  void operator()(size_t i, const PELIRetValue& value) const {  // NOLINT(*)
...
template<typename... Args>
inline PELIRetValue PackedFunc::operator()(Args&& ...args) const {
  const int kNumArgs = sizeof...(Args);
  const int kArraySize = kNumArgs > 0 ? kNumArgs : 1;
  PELIValue values[kArraySize];
  int type_codes[kArraySize];
  detail::for_each(PELIArgsSetter(values, type_codes),
                   std::forward<Args>(args)...);
  PELIRetValue rv;
  body_(PELIArgs(values, type_codes, kNumArgs), &rv);
  return rv;
}
...
inline void unpack_call(const F& f, const PELIArgs& args, PELIRetValue* rv) {
...
template<typename R, typename ...Args>
TypedPackedFunc<R(Args...)>::TypedPackedFunc(const PELIRetValue& value)
    : packed_(value.operator PackedFunc()) {}

template<typename R, typename ...Args>
TypedPackedFunc<R(Args...)>::TypedPackedFunc(const PELIArgValue& value)
    : packed_(value.operator PackedFunc()) {}

template<typename R, typename ...Args>
template<typename FType>
inline void TypedPackedFunc<R(Args...)>::AssignTypedLambda(FType flambda) {
  packed_ = PackedFunc([flambda](const PELIArgs& args, PELIRetValue* rv) {
...
struct PELIValueCast<T, TSrc, true> {
  static T Apply(const TSrc* self) {
...
inline PELIArgValue::operator T() const {
  return detail::
      PELIValueCast<T, PELIArgValue, extension_class_info<T>::code != 0>
      ::Apply(this);
}

template<typename T, typename>
inline PELIRetValue::operator T() const {
  return detail::
      PELIValueCast<T, PELIRetValue, extension_class_info<T>::code != 0>
      ::Apply(this);
}

template<typename T, typename>
inline void PELIArgsSetter::operator()(size_t i, const T& value) const {
...
#endif  // PELI_RUNTIME_PACKED_FUNC_H_
