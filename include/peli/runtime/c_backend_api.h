 ...
 * \file peli/runtime/c_backend_api.h
 * \brief PELI runtime backend API.
 *
 *  The functions defined in this header are intended to be
 *  used by compiled peli operators, usually user do not need to use these
 *  function directly.
 */
#ifndef PELI_RUNTIME_C_BACKEND_API_H_
#define PELI_RUNTIME_C_BACKEND_API_H_
 ...
 *  The user do should not call PELIFuncFree on func.
 *
 ...
 */
PELI_DLL int PELIBackendGetFuncFromEnv(void* mod_node,
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
PELI_DLL int PELIBackendRegisterSystemLibSymbol(const char* name, void* ptr);
 ...
 */
PELI_DLL void* PELIBackendAllocWorkspace(int device_type,
                                       int device_id,
                                       uint64_t nbytes,
                                       int dtype_code_hint,
                                       int dtype_bits_hint);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 *
 * \sa PELIBackendAllocWorkspace
 */
PELI_DLL int PELIBackendFreeWorkspace(int device_type,
                                    int device_id,
                                    void* ptr);

/*!
 * \brief Environment for PELI parallel task.
 */
typedef struct {
  /*!
 ...
 */
typedef int (*FPELIParallelLambda)(
    int task_id, PELIParallelGroupEnv* penv, void* cdata);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
PELI_DLL int PELIBackendParallelLaunch(FPELIParallelLambda flambda,
                                     void* cdata,
                                     int num_task);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
PELI_DLL int PELIBackendParallelBarrier(int task_id, PELIParallelGroupEnv* penv);
 ...
 * \return 0 when no error is thrown, -1 when failure happens
 */
PELI_DLL int PELIBackendRunOnce(void** handle,
                              int (*f)(void*),
                              void *cdata,
                              int nbytes);
...
#endif
#endif  // PELI_RUNTIME_C_BACKEND_API_H_
