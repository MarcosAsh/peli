...
import numpy as np
import peli
from peli import VideoReader
from peli.bridge import *
...
def test_mxnet_bridge():
    try:
        from peli.bridge.mxnet import try_import_mxnet
        ...
        assert isinstance(native_frame, peli.nd.NDArray), type(native_frame)
    except ImportError:
        print('Skip test mxnet bridge as mxnet is not found')

def test_torch_bridge():
    try:
        from peli.bridge.torchdl import try_import_torch
        import torch
        vr = _get_default_test_video()
        with use_torch():
            frame = vr[0]
...
def test_tf_bridge():
    try:
        from peli.bridge.tf import try_import_tfdl
...
def test_tvm_bridge():
    try:
        from peli.bridge.tvm import try_import_tvm
        tvm = try_import_tvm()
        vr = _get_default_test_video()
        with use_tvm():
            frame = vr[0]
...
def test_threaded_bridge():
    # issue #85
    from peli import cpu, gpu
    from multiprocessing.dummy import Pool as ThreadPool

    video_paths = [
      os.path.expanduser('~/Dev/peli/examples/flipping_a_pancake.mkv'), #list of paths to video
      ]
