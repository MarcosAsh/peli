...
class CUDADeviceAPI final : public DeviceAPI {
 public:
  ...
  PELIStreamHandle CreateStream(PELIContext ctx) {
    CUDA_CALL(cudaSetDevice(ctx.device_id));
    cudaStream_t retval;
    CUDA_CALL(cudaStreamCreate(&retval));
    return static_cast<PELIStreamHandle>(retval);
  }

  void FreeStream(PELIContext ctx, PELIStreamHandle stream) {
    CUDA_CALL(cudaSetDevice(ctx.device_id));
    cudaStream_t cu_stream = static_cast<cudaStream_t>(stream);
    CUDA_CALL(cudaStreamDestroy(cu_stream));
  }

  void SyncStreamFromTo(PELIContext ctx, PELIStreamHandle event_src, PELIStreamHandle event_dst) {
    CUDA_CALL(cudaSetDevice(ctx.device_id));
  ...
  void SetStream(PELIContext ctx, PELIStreamHandle stream) final {
...
PELI_REGISTER_GLOBAL("device_api.gpu")
